...
class HS_DOI(BaseOrderInfo, ExtendOrder, DivisionGameOrderInfo):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.division_prices_data = get_hearthstone_divisions_data()
        self.division_prices = [item for sublist in self.division_prices_data for item in sublist]
        self.division_prices.insert(0, 0)

        self.marks_data = get_hearthstone_marks_data()
        self.marks_data.insert(0, [0, 0, 0, 0, 0, 0])
...
class HS_AOI(BaseOrderInfo, ExtendOrder, Arena_V2_GameOrderInfo):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        arena_prices = get_hearthstone_battle_prices()
        self.full_price_val = [round(price * 80, 2) for price in arena_prices]
